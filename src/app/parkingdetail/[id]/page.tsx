  ...
  const renderStars = (score: number) => {
    return (
      <div className="flex text-yellow-400 text-sm">
        {[1, 2, 3, 4, 5].map((star) => (
          <span key={star} className={star <= score ? "" : "text-gray-300"}>★</span>
        ))}
      </div>
    );
  };
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl bg-white p-6 shadow-md">
              ...
              <p className="text-gray-600 leading-relaxed whitespace-pre-line">
                {lot.description || "ไม่มีรายละเอียดระบุไว้"}
              </p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl bg-white p-6 shadow-md">
              ...
              <div className="space-y-4">
                {reviews.length > 0 ? (
                  reviews.map((r, i) => (
                    <div key={i} className={`border-b border-gray-100 pb-4 ${i === reviews.length - 1 ? 'border-none pb-0' : ''}`}>
                      <div className="mb-2 flex items-center gap-3">
                        <div className={`flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br ${avatarGradients[i % avatarGradients.length]} text-white font-bold uppercase`}>
                          {r.username.charAt(0)}
                        </div>
                        <div>
                  ...
                  <p className="text-gray-500 text-sm">ยังไม่มีรีวิวสำหรับที่จอดรถนี้</p>
                )}
              </div>
            </div>
